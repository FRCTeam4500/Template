...
public class FeedforwardSim extends SubsystemBase {
  ...
  /**
   * @param calc A function that takes the current state and the voltage, and modifies the passed in
   *     state to be the state after 0.02 seconds
  ...
  /**
   * Creates a feedforward sim model for a flywheel mechanism. This is also applicable for systems
   * that extend instead of rotating, as long as they aren't affected by gravity. For example, a
   * horizontal elevator.
   *
  ...
  /**
   * Creates a feedforward sim model for a elevator mechanism. This is applicable for systems that
   * extends against a constant force of gravity.
   *
   * <p>The feedforward constants should be obtained via SysId
   *
   ...
   * @param initialState The inital position and velocity of the mechanism.
   */
  public static FeedforwardSim createElevator(
      double kG, double kS, double kV, double kA, State initialState) {
    return new FeedforwardSim(
        (state, volts) -> {
          double staticVolts = Math.signum(state.velocity) * kS;
          double velocityVolts = state.velocity * kV;
  ...
  /**
   * Creates a feedforward sim model for a jointed arm mechanism. This is applicable for systems
   * that rotate vertically, and face different gravitational forces depending on their angle. Note
   * that 0 rotations must corespond to the arm being parallel to the ground
   *
   * <p>The feedforward constants should be obtained via SysId
   *
   * @param kG The voltage need to overcome the gravitational force on the system when the mechanism
   *     is parallel to the ground (0 rotations).
   * @param kS The voltage needed to overcome the friction forces in the system.
   * @param kV The voltage needed to cause a given constant velocity.
   * @param kA The voltage needed to cause a given acceleration.
   ...
   *     rotations/second.
   */
  public static FeedforwardSim createArm(
      double kG, double kS, double kV, double kA, State initialState) {
    return new FeedforwardSim(
        (state, volts) -> {
