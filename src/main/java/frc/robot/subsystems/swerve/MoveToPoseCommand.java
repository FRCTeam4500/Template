...
public class MoveToPoseCommand extends CommandBase {
	...
	private ProfiledPIDController driveController = new ProfiledPIDController(
		...
		0,
		0,
		new Constraints(SwerveConstants.MAX_LINEAR_SPEED_MPS, 4),
		0.02
	);
	...
	private static final LoggedTunableNumber ffMinRadius = new LoggedTunableNumber(
		"DriveToPose/FFMinRadius"
	);
	private static final LoggedTunableNumber ffMaxRadius = new LoggedTunableNumber(
		"DriveToPose/FFMinRadius"
	);
		...
		driveController.reset(
			...
			Math.min(
				...
				-new Translation2d(
					...
					.rotateBy(swerve.getRobotAngle()) // Make Field Velo
					.rotateBy(
						targetPoseSupplier
							.get()
							.getTranslation()
	...
	public void execute() {
		...
		if (
			currentDistance < driveController.getPositionTolerance()
		...
		Logger
			.getInstance()
			.recordOutput("DriveToPose/DistanceMeasured", currentDistance);
		Logger
			.getInstance()
			.recordOutput(
				"DriveToPose/DistanceSetpoint",
				driveController.getSetpoint().position
			);
		Logger
			.getInstance()
		...
		Logger
			.getInstance()
			.recordOutput("Odometry/DriveToPoseGoal", targetPose);
	}
	...
	public boolean isFinished() {
		if (
			...
			withinTolerance(
				driveTolerance.get(),
				Rotation2d.fromDegrees(thetaTolerance.get())
			)
		) {
			return true;
		}
