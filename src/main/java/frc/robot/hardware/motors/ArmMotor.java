...
public class ArmMotor extends SubsystemBase implements Loggable {
  ...
  public void periodic() {
    ...
    double ffVolts = 0;
    if (ff != null) {
      ffVolts = ff.ks * Math.signum(fbVolts) + ff.kg * Math.cos(getRotations() * Math.PI * 2);
    }
    voltageSetter.accept(fbVolts + ffVolts);
  }

  public SysIDCommands getSysID(
      String mechName, double voltageRampRate, double stepVoltage, double timeout) {
    Config config =
        new Config(
  ...
  public static ArmMotor fromTalonFX(
      int canID, Consumer<TalonFX> config, FeedbackController fb, ArmFeedforward ff) {
  ...
  public static ArmMotor fromTalonSRX(
    ...
    return new ArmMotor(
        ...
        name -> {
          HoundLog.log(
              name + "/Velocity", motor.getSelectedSensorVelocity() * 10 * conversionFactor);
          HoundLog.log(name + "/Bus Voltage", motor.getBusVoltage());
        });
  }

  public static ArmMotor fromRealisticSim(FeedbackController fb, ArmFeedforward ff) {
  ...
  public static ArmMotor fromIdealSim(FeedbackController fb) {
